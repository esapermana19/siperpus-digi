            ...
            <div class="grid grid-cols-2">
                ...
                <div class="grid grid-cols-2 mb-4">
                    ...
                    <select name="tahun_terbit" id=""
                        class="text-gray-700 w-full p-2 border border-gray-300 rounded-md outline-none focus:ring-2 focus:ring-blue-500">
                        <option value="">--Pilih Tahun--</option>
                        @foreach (range(date('Y'), 1990) as $tahun)
                            <option value="{{ $tahun }}" @selected($tahun == $buku->tahun_terbit)>
                                {{ $tahun }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select name="kategori_id" id="kategori_id"
                       ...
                       <option value="" class="text-grey-700">--Pilih Kategori--</option>
                         @foreach ($kategori as $k)
                            <option value="{{ $k->id }}" @selected($k->id == $buku->kategori_id)>
                                {{ $k->nama_kategori }}
                            </option>
                        @endforeach
                    </select>
